...
def main():
	...
	parser.add_argument("-p", "--dtype", help=f"ML precision for Q table", default=np.zeros(1).dtype)#{','.join(supported_precisions)}",\
	...
	for data in json_datalist:
		...
		agent = QLearningAgent(action_space, env.observation_space, precision)
		...
		for episode in range(args.num_episodes):
			...
			for timeslot in range(num_timeslots):
				...
				state = env.reset(timeslot, data[1])
				done = False
				steps = 0
